...
# Apach 2.0
# Adapted from wenet/examples/vkw2021/s0/local/run_finetune_5h.sh + run_finetune.sh
...
# data
data=data
dict=exp/20210728_u2pp_conformer_exp/words.txt
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  echo "stage 1: generate wav.scp and compute cmvn"
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    [ ! -f $checkpoint ] && \
      ...
      cp $base_model_dir/*.yaml $dir/0.yaml
